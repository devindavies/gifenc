export default function createStream(initialCapacity = 256) {
	...
	return {
		...
		writeBytes(data: number[], offset = 0, byteLength = data.length) {
	...
	function expand(capacity: number) {
		const prevCapacity = contents.length;
		if (prevCapacity >= capacity) return; // No need to expand, the storage was already large enough.
